...
reg [9:0] cfg_coff_en_r, cfg_data_en_r, rd_result_en_r; //打拍供其他模块使�?
...
wire clk_bm;
wire rst_n_bm;
wire rst_index_bm; //�?始读/取数据时复位index
wire ena_bm;
...
wire rd_result_last; //传�?�从bram_mac处的complete信号为AXI的last信号
wire valid_sf;
...
assign rst_index_bm = cfg_data_en_rise | rd_result_en_rise; //�?始读/取数据时复位index
...
assign rden_bm = rd_result_en_r[9] & (~almost_full_outfifo);
	//rd_result_en_rise若干周期后，�?始读取bram
...
(
	...
	.complete(complete_bm_rd), //到bram的index末尾拉高1�?
	.rden(rden_bm),
...
// stat_reg1[1]==1 表示卷积完成：complete_cw时拉高，rst_index_bm时拉�?
// stat_reg1[2]==1 表示bram结果读取完成：complete_bm_rd时拉高，rst_index_bm时拉�?
//
